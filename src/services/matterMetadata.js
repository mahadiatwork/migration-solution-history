...
const unwrapInvokeResponse = (response) => {
  ...
  if (typeof rawStatusMessage === "string" && rawStatusMessage.trim()) {
    try {
      ...
      // Fall through to the other known response shapes.
    }
  }
...
const flattenLayoutFields = (layout) => {
  const sectionFields = (layout?.sections || []).flatMap(
    (section) => section?.fields || []
  );
  return [...(layout?.fields || []), ...sectionFields];
};
...
const dependencyMapFromValues = (pickListValues = []) => {
  ...
  // Layout metadata can expose `maps: []` on every parent option even when it
  // does not include dependency detail. Do not interpret that shape as a rule
  // that forbids every progress value; the v8 map_dependency response (or the
  // all-options fallback) remains authoritative.
...
export const extractMatterLayoutMetadata = (response, matter = null) => {
  ...
  if (!layout) {
    return { layoutId: getMatterLayoutId(matter), stages: [], progress: [], progressByStage: {} };
  }
...
export const getProgressOptions = (metadata, stage, currentValue) => {
  ...
  // A selected stage must never receive an unrestricted list when Zoho's
  // authoritative dependency is unavailable. Preserve a stored edit value via
  // `currentValue`, but require a real mapped stage before offering changes.
...
/**
 * Read Applications picklist metadata and Current Stage -> Matter Progress
 * dependency. Failures intentionally return empty metadata so the dialog can
 * fall back to the record's stored values without blocking history entry.
 */
